...
(if (not (boundp 'url-http-method)) (setq-default url-http-method nil))
(if (not (boundp 'url-http-data)) (setq-default url-http-data nil))
(if (not (boundp 'url-http-extra-headers)) (setq-default url-http-extra-headers nil))
(if (not (boundp 'oauth--token-data)) (setq-default oauth--token-data nil))
(if (not (boundp 'url-callback-function)) (setq-default url-callback-function nil))
(if (not (boundp 'url-callback-arguments)) (setq-default url-callback-arguments nil))
...
;;(set-face-background hl-line-face "DarkMagenta")
...
* Org
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (setq org-agenda-files '("~/Документы/org/adresat-ToDo.org" "~/Документы/org/google.org" "~/Документы/org/toDo.org"))
...
;; (local-set-key (kbd "C-c C-c") 'org-todo)
    ...
    (lambda () 
    (local-set-key (kbd "r") 'org-agenda-earlier) 
    (local-set-key (kbd "C-c C-c") 'org-agenda-todo)
...
* Swiper, ivy, counsel, anzu and phi-search
  #+BEGIN_SRC emacs-lisp
    ...
    (define-key ivy-minibuffer-map (kbd "M-j") 'left-char)
    (define-key ivy-minibuffer-map (kbd "M-l") 'right-char)
...
* Company 
  #+BEGIN_SRC emacs-lisp
    ...
    ;;              (setq company-begin-commands '(self-insert-command))
     ...
     '(company-tooltip
       ((t (:background "gray18" :foreground "coral" :weight normal))))
     '(company-tooltip-selection
       ((t (:background "gray28" :foreground "coral" :weight bold))))
...
* Projectile 
  ...
  (defun set-project-name-by-file ()
  ...
  (when (featurep 'projectile)
    ...
    (if (file-exists-p (concat (expand-file-name (projectile-project-root)) ".projectile"))
        ;; If that's the case, write their contents into the buffer. 
        (with-temp-buffer
        (insert-file-contents (concat (expand-file-name (projectile-project-root)) ".projectile"))
        (buffer-string))
    ;; If not, then extract the projectname.
    (projectile-project-name))))


      (setq project-name (set-project-name-by-file))
...
* Dired
  ...
  (when (not (require 'dired+ nil 'noerror)) 
    (package-install-file "~/.emacs.d/ab/dired+.el"))
    (require 'dired+)
...
* Multiple cursors
  ...
  :ensure t
  :config
    (define-key xah-fly-leader-key-map (kbd "v") 'mc/mark-more-like-this-extended)  
    (define-key mc/mark-more-like-this-extended-keymap (kbd "C-i") 'mc/mmlte--up)
    (define-key mc/mark-more-like-this-extended-keymap (kbd "C-k") 'mc/mmlte--down)
    (define-key mc/mark-more-like-this-extended-keymap (kbd "C-j") 'mc/mmlte--left)
    (define-key mc/mark-more-like-this-extended-keymap (kbd "C-l") 'mc/mmlte--right)
...
* Keybindings
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (define-key xah-fly-c-keymap (kbd "i") 'bookmark-jump)
    ...
    (define-key xah-fly-leader-key-map (kbd "u") 'imenu-list-smart-toggle)
...
* Indium
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package indium
     :ensure t
     :config
     (setq indium-chrome-executable "/opt/google/chrome/chrome")
...
\n"))
  ...
  ;; (add-hook 'after-save-hook 'ab-indium-refresh-css)
  (defun ab-indium-refresh-css-onsave ()
    "set hook to refresh css "  
    (interactive)
    (add-hook 'after-save-hook 'ab-indium-refresh-css)
  )

  (defun ab-indium-refresh-css ()
    "refresh css in browser without reload page."
    (interactive)
    (indium-eval "{let h, a, f;a = document.getElementsByTagName('link');for (h = 0; h < a.length; h++){f = a[h]; if (f.rel.toLowerCase().match(/stylesheet/) && f.href) {  let g = f.href.replace(/(&|\\?)rnd=\\d+/, '');  f.href = g + (g.match(/\\?/) ? '&' : '?'); f.href += 'rnd=' + (new Date().valueOf()); }};console.log('CSS RELOADED BY Indium');}"))

    (defun ab-indium-reload-onsave ()
    "set hook to refresh css "  
    (interactive)
    (add-hook 'after-save-hook 'ab-indium-reload)
    )

  (defun ab-indium-reload ()
    "refresh css in browser without reload page."
    (interactive)
    (indium-eval "{location.reload();;}"))
...
#+BEGIN_SRC emacs-lisp
   ...
   :commands lsp
      ...
      (define-key xah-fly-comma-keymap (kbd "k") 'lsp-find-references)
      (define-key xah-fly-comma-keymap (kbd "l") 'lsp-find-definition)
      (define-key xah-fly-comma-keymap (kbd "j") 'dumb-jump-back)
      (define-key xah-fly-comma-keymap (kbd "o") 'lsp-rename)
